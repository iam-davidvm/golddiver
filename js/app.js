...
// variables to define if its a default game or a custom 
let isDefault = true;
let treasures = 0;
let gridSize = 0;

// define the place of the default treasure and set the guesses and foundGold to its initial state
const todaysGold = goldTreasures[parseInt((unixToday - dayZero) / 86400)];
let guesses = 1;
let foundGold = false;
...
// define if a user has already played today
...
const showInfo = () => {
    ...
    const btnInfo = modalInfo.querySelector('button');
...
/* end of turn tile action */

/* updating the stats of a player (if its a default game) */
const updateStats = () => {
...
/* the max-width of bar is  80% the other bars are based on that number */
...
/* end of the stats bar rendering */

/* generate random treasures for a custom game */
const generateTreasures = () => {
    let randomTreasures = [];
    for (let i = 0; i < treasures; i++) {
        randomTreasures.push(Math.ceil(Math.random() * gridSize));
    }
    return randomTreasures;
}
/* end of generating random treasures */
...
const gameOver = () => {
    ...
    modalStats.classList.add('modal-reveal');

    const modalStatsQuests = document.querySelector('.modal-stats-quests');
    const modalStatsTreasures = document.querySelector('.modal-stats-treasures');
    modalStatsQuests.innerText = playerStats.gamesPlayed;
    modalStatsTreasures.innerText = playerStats.gamesWon;

    localStorage.setItem('golddiver', JSON.stringify(playerStats));

    // we only show the bars when 1 or more games are won
...
/* if a game is over we reset the grid and initial values */
const resetGame = () => {
...
// check if the info modal has to be shown or not
...
btnSaveSettings.addEventListener('click', () => {
    const radiosTreasures = document.querySelectorAll('input[name="treasures"]');
    for (let radioTreasures of radiosTreasures) {
        if (radioTreasures.checked) {
            treasures = parseInt(radioTreasures.value);
        }
    }
